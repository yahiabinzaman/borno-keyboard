# Borno (বর্ণ) — Avro Phonetic Bangla Keyboard for macOS

<div align="center">
  <img src="./docs/preview.png" alt="Borno Keyboard Banner" width="100%">
</div>

**The only Avro Phonetic keyboard built natively for Apple Silicon Macs.**

Borno brings Avro Phonetic-style Bangla (Bengali) typing to Apple Silicon Macs natively (M1, M2, M3, M4, M5) — no Rosetta required. If you used Avro Keyboard on Windows, iAvro on macOS, or OpenBangla Keyboard on Linux, Borno is your native Apple Silicon alternative.

**[Download](https://github.com/yahiabinzaman/macbangla/releases/latest)** | **[Website](https://yahiabinzaman.github.io/macbangla/)**
...
Borno is built natively for Apple Silicon — no Rosetta required, future-proof as macOS evolves. It works in every app — Safari, Chrome, VS Code, Notes, Spotlight, everywhere.
...
```sh
brew install --cask yahiabinzaman/macbangla/borno
```

That single command auto-taps and installs Borno. Then jump to **step 3** below to add the input source.

To update later: `brew upgrade --cask borno`. To uninstall: `brew uninstall --cask borno`.

### Option B — Download the DMG

1. Download the latest `.dmg` from [Releases](https://github.com/yahiabinzaman/macbangla/releases/latest)
2. Open the DMG and double-click **Install Borno.pkg**

   > **macOS may block the installer** since Borno isn't signed with an Apple Developer ID yet. If you see *"Install Borno.pkg" Not Opened*, follow these two steps:
   >
   ...
   > 
   > **b.** Open **System Settings → Privacy & Security**, scroll to the **Security** section, and click **Open Anyway** next to *"Install Borno.pkg" was blocked*. Then double-click the .pkg again.
   >
   > <img src="./docs/install_allow_anyway.png" alt="Privacy & Security pane — click Open Anyway" width="500">

### Final steps (both options)

3. Go to **System Settings → Keyboard → Input Sources → Edit**, click **+**, find **Borno**, and add it
   > If Borno doesn't appear in the list, log out of your Mac and log back in — macOS sometimes needs this to discover new input methods on first install.
4. Use Globe key or Ctrl+Space to switch to Bangla

## Typing modes

Open the **Settings** tab in the Borno window to pick how typing behaves:

<p align="center">
  <img src="./docs/settings.png" alt="Borno Settings tab — Phonetic-first, Smart suggestions, and Phonetic-only modes" width="640">
</p>

- **Phonetic-first** *(default)* — your exact phonetic spelling is committed by default, but the suggestion list is still one keypress away. Borno remembers the words you deliberately pick.
...
- **Swift IMK layer** (`Borno/`) — subclasses `IMKInputController`, handles key events, candidate window, and text commits
...
Borno is powered by [OpenBangla's riti engine](https://github.com/OpenBangla/riti) — the same Bengali transliteration engine behind [OpenBangla Keyboard](https://github.com/OpenBangla/OpenBangla-Keyboard) on Linux.

## Feedback

Found a bug or have a suggestion? [Open an issue](https://github.com/yahiabinzaman/macbangla/issues).
